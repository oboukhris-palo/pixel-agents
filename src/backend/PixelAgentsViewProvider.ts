...
import { FrameworkConfigService } from './frameworkConfigService.js';
import type { WorkflowState } from './types.js';

// Note: DocumentWatcher system removed - was partially implemented and causing errors
...
export class PixelAgentsViewProvider implements vscode.WebviewViewProvider {
	...
	// Framework Config Service (gene2-core v2.0.1 integration)
	frameworkConfigService: FrameworkConfigService | null = null;
	frameworkConfigOutputChannel: vscode.OutputChannel | null = null;
				...
				(async () => {
					try {
						...
						this.achievementMessageHandler?.on('achievement.unlocked', (achievement: { milestone?: number }) => {
							...
							// Initialize Framework Config Service (gene2-core v2.0.1 integration)
							console.log('[Extension] 🔧 Initializing framework config service...');
							if (!this.frameworkConfigOutputChannel) {
								this.frameworkConfigOutputChannel = vscode.window.createOutputChannel('Pixel Agents: Framework Config');
							}
							
							this.frameworkConfigService = new FrameworkConfigService(
								workspaceRoot,
								this.frameworkConfigOutputChannel
							);
							
							await this.frameworkConfigService.initialize();
							
							this.frameworkConfigService.startMonitoring((message) => {
								if (this.webview) {
	...
	dispose() {
		...
		// Dispose framework config service
		this.frameworkConfigService?.dispose();
		this.frameworkConfigService = null;
		this.frameworkConfigOutputChannel?.dispose();
		this.frameworkConfigOutputChannel = null;
