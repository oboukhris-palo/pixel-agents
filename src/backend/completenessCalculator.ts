...
export class CompletenessCalculator {
	 ...
	 * Tries docs/05-implementation/user-stories.md first; falls back to
	 * docs/01-requirements/user-stories.md if the implementation file is absent.
	 *
	 * @returns Array of story status objects
	 * @private
	 */
	private async parseUserStoriesFile(): Promise<StoryStatus[]> {
		// Primary: implementation tracking file
		try {
			...
			return this.extractStoryStatuses(Buffer.from(content).toString('utf-8'));
		} catch {
			// Fall through to PRD fallback
		}

		// Fallback: PRD requirements file — treat all listed stories as "Not Started"
		try {
			...
			return this.extractStoryStatusesFromPRD(Buffer.from(content).toString('utf-8'));
		} catch (error) {
			this.log(`Error reading user-stories.md (both sources): ${error}`);
			return [];
		}
	}

	/**
	 * Extract story references from a PRD user-stories.md.
	 * PRD files list stories as "### US-NNN: Title" but have no **Status** field,
	 * so every story is treated as 'Not Started'.
	 *
	 * @param text - PRD markdown content
	 * @returns Array of story statuses (all 'Not Started')
	 * @private
	 */
	private extractStoryStatusesFromPRD(text: string): StoryStatus[] {
		const stories: StoryStatus[] = [];
		// Match ### US-NNN: or ### US-NNN-NNN:
		const pattern = /###\s+(US-\d+(?:-\d+)*):/g;
		let match: RegExpExecArray | null;
		while ((match = pattern.exec(text)) !== null) {
			stories.push({ storyRef: match[1], status: 'Not Started' });
		}
		this.log(`PRD fallback: found ${stories.length} stories`);
		return stories;
	}
	...
	startMonitoring(callback: (metrics: ProjectMetrics) => void): void {
		...
		// Watch for file changes in both implementation and PRD user-stories files
		const pattern = new vscode.RelativePattern(
			this.workspaceRoot,
			'{docs/05-implementation/user-stories.md,docs/01-requirements/user-stories.md}'
		);
