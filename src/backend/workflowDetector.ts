...
export class WorkflowDetector {
	...
	/**
	 * Detect the current PDLC phase by finding the HIGHEST-numbered docs phase
	 * folder that contains at least one markdown file.
	 *
	 * CRITICAL FIX v1.0.6: Iterate through ALL candidates and return the HIGHEST
	 * phase with content, not the first match. Old code checked 8→0, returning 8
	 * immediately if 05-implementation existed, even for projects in Phase 3 (testing).
	 *
	 * Folder → phase mapping:
	 *   05-implementation → 8  (highest)
	 *   04-planning       → 6
	 *   03-testing        → 5
	 *   02-architecture   → 3
	 *   01-requirements   → 1
	 *   00-assessment     → 0  (lowest)
	 */
	private detectArtifactBasedPhase(): number {
		const candidates: Array<{ phase: number; folder: string }> = [
			{ phase: 0, folder: path.join(this.workspaceRoot, 'docs', '00-assessment') },
			{ phase: 1, folder: path.join(this.workspaceRoot, 'docs', '01-requirements') },
			{ phase: 3, folder: path.join(this.workspaceRoot, 'docs', '02-architecture') },
			{ phase: 5, folder: path.join(this.workspaceRoot, 'docs', '03-testing') },
			{ phase: 6, folder: path.join(this.workspaceRoot, 'docs', '04-planning') },
			{ phase: 8, folder: path.join(this.workspaceRoot, 'docs', '05-implementation') },
		];

		let highestPhase = 0;
		for (const { phase, folder } of candidates) {
			if (fs.existsSync(folder)) {
				try {
					const hasMarkdown = fs.readdirSync(folder).some(f => f.endsWith('.md'));
					if (hasMarkdown) { 
	...
	private detectPDLCStage(): { 
		...
		const stage = this.detectArtifactBasedPhase();
