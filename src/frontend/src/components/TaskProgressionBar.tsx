...
export interface TaskProgressionBarProps {
  ...
  completeness?: number;
  /**
   * Current PDLC phase number (0=Assessment, 1-2=Requirements, 3-4=Architecture,
   * 5=Testing, 6-7=Planning, 8=Implementation). Drives the phase pill label.
   * Optional — defaults to '8 Impl' when not provided.
   */
...
const PHASE_DESCRIPTIONS: Readonly<Record<number, string>> = {
...
export const TaskProgressionBar = memo(function TaskProgressionBar({
  ...
  return (
    ...
    >
      ...
      {/* Phase Pill - 80×24px pill showing current PDLC phase */}
      <span
        className={styles.phasePill}
        data-testid="phase-pill"
        title={
          workflowPhase !== undefined
            ? `PDLC Phase ${workflowPhase}: ${PHASE_DESCRIPTIONS[workflowPhase] ?? 'Implementation'}`
            : 'Current phase'
        }
        style={{ cursor: 'help' }}
      >
        {workflowPhase !== undefined
          ? (PHASE_LABELS[workflowPhase] ?? '8 Impl')
