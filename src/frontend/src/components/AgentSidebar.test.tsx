 ...
 * - AC-S4: TDD sub-agents (dev-tdd-red, dev-tdd-green, dev-tdd-refactor) are filtered out (not rendered)
 */
...
// Note: Using 'qa' as example disabled agent (not TDD sub-agent which are filtered out)
...
describe('AgentSidebar Component (v1.0.5 enhancements)', () => {
  ...
  describe('AC-S2: Disabled agent visual rendering', () => {
    it('renders ⛔ icon for disabled agent', () => {
      ...
      expect(screen.getByTestId('agent-disabled-icon-qa')).toBeInTheDocument();
      expect(screen.getByTestId('agent-disabled-icon-qa')).toHaveTextContent('⛔');
    });

    it('applies disabled CSS class to disabled agent row', () => {
      render(<AgentSidebar agents={[DISABLED_AGENT]} />);
      const row = screen.getByTestId('agent-row-qa');
  ...
  describe('AC-S4: TDD sub-agent filtering', () => {
    it('filters out TDD sub-agents (dev-tdd-red, dev-tdd-green, dev-tdd-refactor)', () => {
      const agents: AgentInfo[] = [
        ...
        { name: 'tdd-orchestrator', status: 'idle' as AgentInfo['status'], isCurrent: false },
      ];
      render(<AgentSidebar agents={agents} />);

      // Active/idle regular agents should appear
      expect(screen.getByTestId('agent-row-orchestrator')).toBeInTheDocument();
      expect(screen.getByTestId('agent-row-architect')).toBeInTheDocument();
      expect(screen.getByTestId('agent-row-tdd-orchestrator')).toBeInTheDocument();

      // TDD sub-agents should NOT appear (filtered out)
      expect(screen.queryByTestId('agent-row-dev-tdd-red')).not.toBeInTheDocument();
      expect(screen.queryByTestId('agent-row-dev-tdd-green')).not.toBeInTheDocument();
      expect(screen.queryByTestId('agent-row-dev-tdd-refactor')).not.toBeInTheDocument();
    });

    it('shows ⛔ icon only for disabled agents (not filtered out)', () => {
      const agents: AgentInfo[] = [
        ACTIVE_AGENT,
        DISABLED_AGENT, // qa agent with disabled status
      ];
      render(<AgentSidebar agents={agents} />);
